...
// Settlementdetails represents the resource /{merchantId}/services/settlementdetails/{paymentId} - Get Settlement details
// Documentation can be found at https://epayments-api.developer-ingenico.com/s2sapi/v1/en_US/go/services/settlementdetails.html
//
// Can return any of the following errors:
// * ValidationError if the request was not correct and couldn't be processed (HTTP status code 400)
// * AuthorizationError if the request was not allowed (HTTP status code 403)
...
// or the service that you're trying to reach is temporary unavailable (HTTP status code 500, 502 or 503)
// * APIError if the Ingenico ePayments platform returned any other error
func (c *Client) Settlementdetails(paymentID string, context communication.CallContext) (services.SettlementDetails, error) {
	var resultObject services.SettlementDetails

	pathContext := map[string]string{
		"paymentId": paymentID,
	}

	uri, err := c.apiResource.InstantiateURIWithContext("/v1/{merchantId}/services/settlementdetails/{paymentId}", pathContext)
	if err != nil {
		return resultObject, err
	}
